...
class Sprite
{
    public:
        ...
        void init(AssetLoader asset_loader, const char* path, const glm::mat4& projection)
        {
            loadTexture(asset_loader, path);

            shader = new Shader(asset_loader, "res/shaders/sprite_vert.glsl", "res/shaders/sprite_frag.glsl");
            shader->use();
    ...
    private:
        ...
        void loadTexture(AssetLoader asset_loader, const char* path)
        {
            // Generate a texture
...
//            image = loadImage(path, &width, &height, &channels_count);
            image = stbi_load(path, &width, &height, &channels_count, 0);

            if (!image)
            {
                __android_log_print(ANDROID_LOG_ERROR, "APP", "Failed to load texture %s\n", path);
            }
